...
def init_database(request,test_client):
    ...
    # Students
    john_smith = Student(username='john_smith', email='js@smith.com', firstname='John', lastname='Smith', gpa=2.2, id=100)
    ...
    dr_grace_hopper = Faculty(username='dr_grace_hopper', email='grace@hopper.com', firstname='Grace', lastname='Hopper', id=68)
    ...
    # Positions
    pos_research_assistant = Position(name='Research Assistant', description='Must have unspoken rizz.', faculty_id=dr_grace_hopper.id)
    pos_teaching_assistant = Position(name='Teaching Assistant', description='Whar', faculty_id=dr_alan_turing.id)
    
    # Course Enrollments
    enroll_jane = CourseEnrollment(student_id=jane_doe.id, course_id=course_intro_cs.id, instructor_id=dr_grace_hopper.id, grade=3)
    enroll_john = CourseEnrollment(student_id=john_smith.id, course_id=course_adv_algo.id, instructor_id=dr_grace_hopper.id, grade=4)
    ...
    rec_jane = Recommendation(student_id=jane_doe.id, faculty_id=dr_grace_hopper.id, application_id=app_jane.id, status='Approved')
